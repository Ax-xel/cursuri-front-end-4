...
for (var i =0; i< user.length; i++) {
	const DataUser = user[i];
	let list = document.getElementById("container");
 	let productName = document.createElement("h3");
  productName.innerHTML = DataUser.name;
	list.appendChild(productName);
// 	console.log(productName.innerHTML);
	
	var imageProd = document.createElement("img");
	imageProd.setAttribute("src", DataUser.imageUrl);
	list.appendChild(imageProd);
	
	let quantity = document.createElement("p");
  quantity.innerHTML = DataUser.quantity;
	list.appendChild(quantity);
	
	let price = document.createElement("p");
  price.innerHTML = DataUser.price;
